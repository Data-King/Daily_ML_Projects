...
class GrocerySalesForecastSystem:
    ...
    def visualize_data(self):
        ...
        # Time series plot
        axes[0, 0].plot(self.df['date'], self.df['sales'], linewidth=1, alpha=0.7)
        axes[0, 0].set_title('Daily Sales Over Time', fontsize=14, fontweight='bold')
        ...
        sales_by_day = self.df.groupby('day_of_week')['sales'].mean()
        axes[0, 1].bar(range(7), sales_by_day, color='skyblue', edgecolor='navy')
        ...
        sales_by_month = self.df.groupby('month')['sales'].mean()
        axes[1, 0].bar(range(1, 13), sales_by_month, color='lightcoral', edgecolor='darkred')
        ...
        # Sales distribution
        axes[1, 1].hist(self.df['sales'], bins=50, color='lightgreen', edgecolor='darkgreen')
        axes[1, 1].set_title('Sales Distribution', fontsize=14, fontweight='bold')
        axes[1, 1].set_xlabel('Sales ($)')
        axes[1, 1].set_ylabel('Frequency')
        axes[1, 1].axvline(self.df['sales'].mean(), color='red', 
                          linestyle='--', label=f'Mean: ${self.df["sales"].mean():.2f}')
        axes[1, 1].legend()
        axes[1, 1].grid(True, alpha=0.3, axis='y')
        
        plt.tight_layout()
        plt.savefig('sales_visualization.png', dpi=300, bbox_inches='tight')
        print("Sales visualization saved as 'sales_visualization.png'")
        plt.show()
    ...
    def plot_model_comparison(self, results):
        ...
        # MAE comparison
        axes[0].bar(models, test_mae, color=['skyblue', 'lightcoral', 'lightgreen'])
        axes[0].set_title('Test MAE by Model', fontsize=14, fontweight='bold')
        ...
        # R² comparison
        axes[1].bar(models, test_r2, color=['skyblue', 'lightcoral', 'lightgreen'])
        axes[1].set_title('Test R² Score by Model', fontsize=14, fontweight='bold')
    ...
    def plot_predictions(self, y_test, predictions, dates_test):
        ...
        axes[0].plot(dates_test, y_test.values, label='Actual Sales', 
                    linewidth=2, marker='o', markersize=3, alpha=0.7)
        axes[0].plot(dates_test, predictions, label='Predicted Sales', 
        ...
        # Last 30 days zoom
        if len(dates_test) > 30:
            axes[1].plot(dates_test[-30:], y_test.values[-30:], label='Actual Sales',
                        linewidth=2, marker='o', markersize=5, alpha=0.7)
            axes[1].plot(dates_test[-30:], predictions[-30:], label='Predicted Sales',
                        linewidth=2, marker='s', markersize=5, alpha=0.7)
            axes[1].set_title('Last 30 Days - Detailed View', fontsize=14, fontweight='bold')
        else:
            axes[1].plot(dates_test, y_test.values, label='Actual Sales',
                        linewidth=2, marker='o', markersize=5, alpha=0.7)
            axes[1].plot(dates_test, predictions, label='Predicted Sales',
                        linewidth=2, marker='s', markersize=5, alpha=0.7)
            axes[1].set_title('Test Set - Detailed View', fontsize=14, fontweight='bold')
...
if __name__ == "__main__":
    ...
    forecaster.plot_model_comparison(results)
    
    # Step 7: Plot predictions
    split_idx = int(len(forecaster.df) * 0.8)
    dates_test = forecaster.df['date'].iloc[split_idx:]
    forecaster.plot_predictions(y_test, results[best_model]['predictions'], dates_test)
    
    # Step 8: Feature importance
    forecaster.plot_feature_importance()
    
    # Step 9: Forecast next day
