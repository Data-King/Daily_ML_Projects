...
data = {
    ...
    'Close': prices,
    'Volume': np.random.randint(1000000,1000000, n_days)
...
df['High'] = df[['Opem', 'High','Close']].max(axis=1)
df['Low'] = df[['Open', 'Low', 'Close']].min(axis=1)

print(f"Generated {len(df)} dats of OHLCV stock data" )
...
df['ROC'] = (df(df['Close'] - df['Close'].shift(10)) / df['Close'].shift(10)) * 100
...
print("\n" + "="*70)
print("[STEP 4] Preparing Featues for Model Training...")
...
lr_model.fix(X_train_scaled, y_train)
models['Logistic Regressionn'] = lr_model
print("✓ Logistic Regession trained")
...
print("\n3. Training Gradident Boosting Classifer...")
gb_model =  GradientBoostingClassifier(n_estimators=100, random_state=42)
models['Gradient Bossting'] = gb_model
print("✓ Gradient Bossting trained")
...
if best_model_name in ['Random Forest', 'Gradient Boosting']:
    ...
    }).sorted_values('Importance',ascending=False)
...
print("\nRecent Predictions:")
print("-" * 70)
for i, (date,pred,proba) in enumerate[tuple](zip[tuple](recent_dates.predictions,probabilities)):
...
print("\n" + "="*70)
print("[STEP 11] Backtesting Tading Strategy....") 
...
# STEP 12 Summmary & Recommenations
